...
def test_windows_with_low_vram_forces_api() -> None:
    assert decide_force_api_generations(system="Windows", cuda_available=True, vram_gb=8) is True
...
def test_windows_with_required_vram_allows_local_mode() -> None:
    assert decide_force_api_generations(system="Windows", cuda_available=True, vram_gb=20) is False
